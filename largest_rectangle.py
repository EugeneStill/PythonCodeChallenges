import unittest


class LargestRectangleArea(unittest.TestCase):
    """
    Given an array of integers heights representing the histogram's bar height where the width of each bar is 1,
    return the area of the largest rectangle in the histogram.

    Input: heights = [2,1,5,6,2,3]
    Output: 10
    Explanation: The above is a histogram where width of each bar is 1.
    The largest rectangle is shown in the red area, which has an area = 10 units.
    """
    def largest_rectangle_area(self, heights):
        """
        :type heights: List[int]
        :rtype: int
        """
        res, past_bars = 0, []
        heights = [0] + heights + [0]

        for right, current_height in enumerate(heights):
            while past_bars and heights[past_bars[-1]] > current_height:
                prev_height = heights[past_bars.pop()]
                left = past_bars[-1]
                # subtract 1 because the width of each bar is 1
                area = (right-left-1) * prev_height
                res = max(res, area)
            past_bars.append(right)
        return res

    def test_lra(self):
        self.assertEqual(self.largest_rectangle_area([2,1,5,6,2,3]), 10)

